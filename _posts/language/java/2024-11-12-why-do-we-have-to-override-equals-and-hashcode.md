...
`hashcode()` 는 해시 기반 자료구조에서 관리하는데 사용하는 **해시 값을 반환**해주는 메서드이다. 
...
왜냐하면  **해시 기반 컬렉션**에서 **정상적으로(+개발자의 의도대로) 동작**하는 것을 보장하기 위해서 두 메서드를 일관되게 재정의해야만 한다.

해시 기반 컬렉션의 **동작과정**에서 `equals()`와 `hahscode()`를 활용하기 때문이다. 

그렇지 않으면 해시 기반 컬렉션을 다룰 때 전혀 예상하지 않은 문제 마주할 수 있다.  

예상하지 못한 **문제의 발생 이유**와,  `equals()` 와 `hashcode()` 를 **일관성 있게 재정의하지 않았을 때** 발생할 수 있는 문제점을 알아본다. 

### 2.1 해시 기반 컬렉션 동작방식에 기인한 문제 발생 

문제가 발생하는 가장 큰 원인은, 해시 기반 컬렉션의 동작방식과 깊은 연관이 있다. 


(그림)

`Java`의 대표적 자료구조인 `hashMap` 은 다음과 같이 동작한다.

```java
public static void main(String[] args) {
    Map<Product, Money> priceTable = new HashMap<>();
    priceTable.add(); // 
}
```
...
2. 해당 버킷에 이미 객체가 존재하는 경우(충돌의 경우), -> equals 메서드를 비교
	1. 만일 동일하면 중복 객체로 판단
	2. 그렇지 않으면 새로운 객체 저장

더 자세한 내용은 👉 [해시 자료구조와 Java HashMap이 해시 충돌을 해결하는 방법](https://dongmin-sim.github.io/posts/hash-data-structure/) 참고하자. 
...
이를 막기 위해 **지켜야하는 규칙**들을 살펴본다.

앞서 살펴본 문제들을 예방하기 위해 자바에서는 `equals()`와 `hashcode()`의 **일관된 재정의 규칙**을contract로 명시해두었다. 이 내용들은 java 공식 문서에 있는 내용들이며, 다음과 같다.
