...
public static void main(String[] args) {
	...
	Money price = new Money(15000, "KRW");
    
    priceTable.put(book, price); // 데이터 삽입
}
```
...
- 만일, 해당 버킷에 이미 객체가 존재하는 경우 (해시충돌이 발생한 경우)
	- 만일 동일하면 중복 객체로 판단
	- 순회 결과 동일한 객체가 없다면, 새로운 객체 저장

충돌이 발생했을 경우, 버킷에 이미 존재하는 객체와 새로 저장하는 객체를 비교하게 되는데   
이때 `equals()` 메서드를 가지고 비교하게 된다. 

(순서도 발생 그림)
동일한 객체인지 -> 동일하다면, 수정, 다르다면 삽입
