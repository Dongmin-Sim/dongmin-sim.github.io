...
해시함수에서 서로 다른 `key`에 대해 동일한 해시값을 반환하여, 버킷의 인덱스 주소가 같아져버리는 상황을 해시충돌이라한다. 해시 충돌이 발생할 경우 다음과 같은 순서로 해시 충돌을 

- 만일, 충돌이 발생한 버킷에 이미 `Node`가 존재하는 경우. **동일 비교 시행** 
- 버킷에 연결된 모든 노드를 순차 탐색하면서 기존 값과 새로 삽입하려는 값과 **동일 비교 시행** [^fn-nth-1]
	- (업데이트) 만일 동일하면 중복 객체로 판단 후 새로운 `value`로 수정.
	- (새로삽입) 순회 결과 동일한 객체가 없다면, 새로운 객체 저장.

이때 **동일 비교 시행**하는 방법으로 아래의 조건문을 사용한다.
이 조건의 결과가 참인 경우는, `hashMap`에 있던 노드의 값과 새로 삽입하려는 값이 같다는 것을 의미한다.
```java
...
![Desktop View](/assets/posts/language/java/2024-11-12-why-do-we-have-to-override-equals-and-hashcode/push.png){: width="972" height="589"}_`HashMap` 의 `putVal` 과정 흐름도_

해시 충돌 발생시  `equals()` 와 `hashcode()`를 통해 노드간 동일 비교를 하게 된다.

때문에 `hashcode()` 와 `equals()` 를 일관되게 재정의해야 한다. 그렇지 않으면 해시 기반 컬렉션을 다룰 때 전혀 예상치 못한 문제와 마주할 수 있다.  
다음에는 `equals()` 와 `hashcode()` 를 일관성 있게 재정의하지 않았을 때 발생할 수 있는 문제점을 알아본다. 

### 2.2 두 메서드를 잘못 오버라이드하면?

#### equals를 잘못 오버라이드하면?
동등한 값이어도 equals()가 참이 아니라면 어떻게 될까?
(이미지)
- 논리적으로 동일함을 보장하지 못한다면, 중복 값이 저장될 수 있다. 
- 검색은 되지만, 같은 해시값을 가지는 버킷에 중복 저장되어, 다만 버킷의 길이가 길어지게 됨. 

#### hashcode를 잘못 오버라이드하면?
동등한 값이어도 다른 hascode가 나온다면 어떻게 될까?

(이미지)
- 이 경우 역시 논리적으로 동일한 두 객체가 중복 저장될 수 있다. 
- 두 객체가 다른 해시 코드를 가져 다른 버킷에 저장될 수 있다. 이 경우 검색이 제대로 작동하지 않는다.

## 3. equals()와 hashCode()의 재정의 규칙

앞서 살펴본 문제들을 예방하기 위해 자바에서는 `equals()`와 `hashcode()`의 **일관된 재정의 규칙**을 contract로 명시해두었다. 이 내용들은 java 공식 문서에 있는 내용들이며, 다음과 같다.
...
`hashCode()`는 별도로 재정의하지 않았다. [^fn-nth-2]
```java
...
따라서 `equals()` 가 true를 반환하는 두 객체는 반드시 같은 `hashCode()`값을 가져야만 한다.
...
즉, 다른 객체가 동일한 해시 코드를 가질 수 있지만, 최대한 줄이는 것이 좋다. 왜냐하면 해시 충돌(hash collision)을 초래할 수 있기 때문이다. 충돌이 많아질수록 성능이 저하될 가능성이 있다.
...
다음 주석은 `hashCode()`에 적힌 내용으로. Java의 `Object` 클래스에 정의된 `hashCode` 메서드가 가능한 한 실용적인 범위 내에서 서로 다른 객체에 대해 서로 다른 정수 값을 반환하도록 설계되었음을 이야기한다. 

### 3.3 **동일한 객체는 항상 동일한 `hashCode()` 값을 반환해야 함.**

프로그램 실행 중에 객체의 상태가 변경되지 않았다면, 동일한 객체는 항상 같은 해시 코드를 반환해야 한다. 
만일 동일한 객체임에서 서로 다른 `hashCode()` 를 반환할 경우 앞서 살펴보았던 "`hashCode()`를 잘못 오버라이드하면?"처럼 저장했던 객체가 중복저장되거나, 조회할 수 없게 될 수 있다. 
...
`equals()` 와 `hashcode()` 를 일관되게 재정의해야하는 이유에 대해서 알아보았다. 
글에서는 



[^fn-nth-1]: Java의 `HashMap`은 충돌이 발생할 경우 일정 비율까지는 링크드 리스트 구조를 가지는 체이닝 방식으로 충돌을 해결한다. 하지만 Java JDK 1.8 이후부터는 일정 비율을 넘어갈 경우, 관리 효율을 위해 레드-블랙 트리로 변환하는 과정을 거친다. 순차탐색 이전에 링드리스트인지, 레드-블랙트리인지 확인하는 조건문 결과에 따라 서로 다른 방식으로 값을 순회한다.
[^fn-nth-2]: `hashCode` 를 재정의하지 않을 경우 객체 고유의 식별자를 반환한다. 