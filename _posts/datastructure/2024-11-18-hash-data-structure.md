...
"해시 충돌 가능성을 최소화 해야한다" 의 조건을 알아보기 이전에 해시 충돌에 대해서 알아보자.
...
이 방법은 빈 버킷을 찾기 위해서 계속 탐색을 해야하기 때문에, 해시 테이블에 데이터가 저장된 적재율이 높아질수록 빈 버킷을 찾기 위해 더 많은 탐색이 필요해져 성능이 저하된다.

### 좋은 해시 함수의 조건: 많은 해시 충돌 발생 시
앞에서 좋은 해시함수의 조건으로 "해시 충돌을 최소화해야 한다"고 했음.

충돌 해결방법을 사용한다고 해서, 모든 문제가 사라지는 것은 아님. 

결론 -> 해시 테이블 성능이 저하됨.

예시, 체이닝 방식에서 해시 충돌이 많이 발생할 경우 
...
때문에 선제적으로 해시함수 자체에서 충돌을 발생하지 않도록 하는것이 더 중요함. 
